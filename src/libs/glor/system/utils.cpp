...
cLoggerStreamEmpty g_nullstream; // extern a stream that does nothing (eats/discards data)
...
// ====================================================================
cLoggerStream::cLoggerStream(const std::string & filename)
:
...
bool cLoggerStream::PrintIsNotEmpty() {
	if (m_filename == "tmp_log") {
		return false;
	}
	else {
		return true;
...
void cLoggerStream::Print(const std::string & str) {
	if (m_use_regular_files) {
...
cLogger::cLogger() :
mUseRegularFiles(false),
mMainMsgQueue(boost::interprocess::open_or_create, MAIN_QUEUE_NAME, 1000, 1000),
mStream(NULL),
...
{
	mStream = new cLoggerStreamCout();
	mStreamBrokenDebug = new cLoggerStreamCerr(); // the backup stream
...
void cLogger::SetStreamBroken(const std::string &msg) {
	...
	if (!mIsBroken) { // if not already marked as broken
		...
		if (mStreamBrokenDebug == nullptr) {
			...
			(*mStreamBrokenDebug) << OT_CODE_STAMP << "WARNING: due to debug stream problem ("<<msg<<") - switching back to fallback stream (e.g. cerr)" << "\n";
		}
		mIsBroken = true;
	}
}

cLoggerStream & cLogger::write_stream(int level) {
	return write_stream(level,"");
}

cLoggerStream & cLogger::write_stream(int level, const std::string & channel ) {
...
void cLogger::OpenNewChannel_(const std::string & channel) { // channel=="net/sleep"
	...
	cLoggerStream * thefile = new cLoggerStream( fname_system); // file system
...
void cLogger::setOutStreamFile(const string &fname) { // switch to using this file
	...
	mOutfile =  make_unique<cLoggerStream>(fname);
	mStream = & (*mOutfile);
...
void cLogger::setOutStreamFromGlobalOptions() {
	if ( gRunOptions.getDebug() ) {
		if ( gRunOptions.getDebugSendToFile() ) {
			mOutfile =  make_unique<cLoggerStream> ("debuglog.txt");
			mStream = & (*mOutfile);
		}
		else if ( gRunOptions.getDebugSendToCerr() ) {
			//mStream = & std::cerr; // TODO
		}
		else {
